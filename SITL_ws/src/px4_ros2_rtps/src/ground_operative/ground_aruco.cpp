...
*/
		...
		auto udpvideo_timer_callback = [this]()->void {
			//std::cout << "Video Checker loop!" << std::endl;
			//std::cout << rx_heartbeat << std::endl;
			/*
			if (rx_heartbeat == 2){ // OBC on network, lets grab UDP
				//cap.release(); //-- supuestamente no es necesario
				pipeline = rxudp;
				if (noUDPflag == 0){
					cap.release();
				...
				if(!cap.isOpened()) { // check if not opened already
					cap.open(pipeline, cv::CAP_GSTREAMER); // then open
				}
			}
			else{ // No OBC on network, grab other video for testing purposes
				pipeline = rxwebcam;
				if(!cap.isOpened()) { // check if not opened already
					cap.open(pipeline, cv::CAP_GSTREAMER); // then open
					noUDPflag = 0;
					...
					std::cout << "Video already opened!" << std::endl;
				};
				
			};
			/
			if(!cap.isOpened()) { // If still no video input
				std::cout<<"WARNING: Unable to open any video input."<<std::endl;
			};
			//int dummy = 1;
			/*
			if(!cap.isOpened()) {
				std::cout<<"Unable to open UDP video input."<<std::endl;
			}
			else{
				std::cout << "UDP Video input opened!" << std::endl;
			};
			//std::cout << "Video input started!" << std::endl;
			*/
			if (!cap.read(image)) { 
				 //Refresh the screen with black image only
				image_black.copyTo(image_copy2);
				if (rx_heartbeat == 1) { // Only refresh if OBC is on network
					  //Try to capture video input from UDP
					image.copyTo(image_copy2);
				}
			}else{
				image_black.copyTo(image_copy2);
				if (rx_heartbeat == 1) { // Only refresh if OBC is on network
				image.copyTo(image_copy2);;  //Try to capture video input from UDP
				}
			};	
		};
	// Main callback function of the node:
	timer_ = this->create_wall_timer(16ms, timer_callback);
	udpvideo_timer_ = this->create_wall_timer(30ms, udpvideo_timer_callback);
	}
private:
	...
	rclcpp::TimerBase::SharedPtr udpvideo_timer_;
